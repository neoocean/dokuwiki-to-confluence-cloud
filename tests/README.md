...
현재 164 통과. 각 파일별로 무엇을 보호하는지 한눈에. 회귀 작성·확장 시
*어느 파일에 추가할까* 결정 가이드.
...
| `test_calendar.py` | 14 | monthcal fallback + Google Calendar iframe + encryptedpasswords (5종) | `_convert_monthcal_fallback / _convert_google_calendar_iframe / _convert_encrypted_passwords / _preprocess_encrypted_passwords` |
...
| `test_dev_plugins.py` | 9 | dev 컨테이너 plugin 자동 감지/설치 + sanity check | `_dev_detect_plugins / _dev_install_plugins` (§ dev) |
