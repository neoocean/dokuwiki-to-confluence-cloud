...
| 10 | 빈 `ri:filename=""` 의 `ac:link` 가 1 페이지 (u:neoocean:2020) 의 *code 블록 안 사용자 예제* 에서 잔존. Confluence 가 빈 filename ri:attachment 를 500 INTERNAL_SERVER_ERROR 로 거부 (split-oversize chunk 시점에 발견) | 변환기가 dokuwiki `[[/_media/...]]` 같은 *internal media URL* 을 첨부 link 로 변환 시도 → 파일명 추출 실패 (빈 string). `_sanitize_empty_attachment_links` 가 split-oversize 시점에만 적용되어 일반 convert 흐름에는 누락 | 변환기 (`_convert_html_to_storage`) 의 serialize 직후에 `_sanitize_empty_attachment_links` 호출 추가 — 빈 ri:filename 의 ac:link 를 *link-body 텍스트 (평문)* 로 격하. 영향 페이지 0 으로 재확인 (CL TBD, 2026-05-23) |
...
## 10c. 라이브 마이그레이션 실행 결과 기록

2026-05-18 첫 라이브 실행 결과는 별도 운영 로그
[`docs/migration-result.md`](migration-result.md). 통계(1,674 페이지
UPLOADED / 10,613 첨부 / 5,180 링크 resolved), audit 3회 진행, 그
